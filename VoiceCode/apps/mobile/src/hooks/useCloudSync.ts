...
import {
  ...
  getTranscriptions,
  getTranscription,
  searchTranscripts as supabaseSearchTranscripts,
...
export function useCloudSync(options: UseCloudSyncOptions = {}): UseCloudSyncReturn {
  ...
  const loadTranscripts = useCallback(async () => {
    ...
    try {
      ...
      if (offlineFirst || !isOnline) {
        ...
        const currentUser = await getCurrentUser();
        if (currentUser) {
          data = await getTranscriptions(currentUser.id, 50) as Transcript[];
        } else {
          data = [];
        }
      } else {
  ...
  const signUp = useCallback(async (email: string, password: string, fullName?: string) => {
    try {
      await supabaseSignUp(email, password, fullName ? { full_name: fullName } : undefined);
      setIsAuthenticated(true);
      await loadUserProfile();
    } catch (err) {
  ...
  // Get transcript
  const getTranscriptById = useCallback(async (id: string): Promise<Transcript | null> => {
    if (offlineFirst) {
      return offlineStorage.current.getTranscript(id);
    } else if (isAuthenticated) {
      return getTranscription(id) as Promise<Transcript>;
    }
    return null;
  ...
  const searchTranscripts = useCallback(async (query: string): Promise<Transcript[]> => {
    if (offlineFirst) {
      ...
      const currentUser = await getCurrentUser();
      if (currentUser) {
        return supabaseSearchTranscripts(currentUser.id, query) as Promise<Transcript[]>;
      }
    }
    return [];
  }, [isAuthenticated, offlineFirst]);
  ...
  return {
    ...
    error,
    saveTranscript,
    getTranscript: getTranscriptById,
    updateTranscript,
    deleteTranscript,
    searchTranscripts,
    refreshTranscripts,
