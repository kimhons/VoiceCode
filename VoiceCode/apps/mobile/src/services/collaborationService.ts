...
import { getSupabaseService, getCurrentUser } from './supabase.service';
...
export class CollaborationService {
  ...
  async createWorkspace(name: string, description?: string): Promise<Workspace> {
    ...
    const { data, error } = await this.supabase.from('workspaces')
      .insert({
        name,
        description,
        owner_id: user.id,
    ...
    // Add owner as member
    await this.supabase.from('workspace_members')
      .insert({
  ...
  async deleteWorkspace(id: string): Promise<void> {
    ...
    const { error } = await this.supabase.from('workspaces')
  ...
  async inviteMember(workspaceId: string, email: string, role: Role): Promise<Member> {
    ...
    // Find user by email
    const { data: userData, error: userError } = await this.supabase.from('user_profiles')
      .select('id')
      .eq('email', email)
      .single();
    ...
    // Add member
    const { data, error } = await this.supabase.from('workspace_members')
      .insert({
  ...
  async removeMember(workspaceId: string, userId: string): Promise<void> {
    ...
    const { error } = await this.supabase.from('workspace_members')
      .delete()
  ...
  async shareTranscript(
    ...
    const { data, error } = await this.supabase.from('shared_transcripts')
      .insert({
  ...
  async unshareTranscript(transcriptId: string, workspaceId: string): Promise<void> {
    ...
    const { error } = await this.supabase.from('shared_transcripts')
      .delete()
  ...
  async addComment(transcriptId: string, content: string, timestamp?: number, parentId?: string): Promise<Comment> {
    ...
    const { data: transcript } = await this.supabase.from('transcripts')
      .select('user_id')
      .eq('id', transcriptId)
      .single();
  ...
  async addAnnotation(
    ...
    const { data, error } = await this.supabase.from('annotations')
      .insert({
        transcript_id: transcriptId,
        user_id: user.id,
  ...
  async deleteAnnotation(annotationId: string): Promise<void> {
    ...
    const { error } = await this.supabase.from('annotations')
      .update({ is_deleted: true })
      .eq('id', annotationId)
      .eq('user_id', user.id);
  ...
  async getUnreadCount(): Promise<number> {
    ...
    const { count, error } = await this.supabase.from('notifications')
  ...
  async getActivity(workspaceId: string, limit: number = 50): Promise<Activity[]> {
    const { data, error } = await this.supabase.from('activity_log')
      .select(`
        *,
        user:user_profiles(id, email, full_name, avatar_url)
      `)
