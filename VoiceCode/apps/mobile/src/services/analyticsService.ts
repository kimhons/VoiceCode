...
import { getSupabaseService, Transcript, getCurrentUser } from './supabase.service';
...
export class AnalyticsService {
  ...
  async getAudioStats(startDate: Date, endDate: Date): Promise<AudioStats> {
    ...
    const { data, error } = await this.supabase.from('analytics_events')
      .select('*')
      .eq('user_id', user.id)
  ...
  async getPerformanceMetrics(startDate: Date, endDate: Date): Promise<PerformanceMetrics[]> {
    ...
    const { data, error } = await this.supabase.from('performance_metrics')
      .select('*')
      .eq('user_id', user.id)
      .gte('date', startDate.toISOString().split('T')[0])
      .lte('date', endDate.toISOString().split('T')[0])
  ...
  async getCostBreakdown(startDate: Date, endDate: Date): Promise<CostBreakdown[]> {
    ...
    const { data, error } = await this.supabase.from('cost_tracking')
      .select('*')
      .eq('user_id', user.id)
      .gte('date', startDate.toISOString().split('T')[0])
      .lte('date', endDate.toISOString().split('T')[0])
  ...
  async getStorageUsage(): Promise<{ total_gb: number; cost: number }> {
    ...
    const { data, error } = await this.supabase.from('transcripts')
      .select('content')
      .eq('user_id', user.id)
      .eq('is_deleted', false);
  ...
  async getDashboardMetrics(): Promise<DashboardMetrics> {
    ...
    // Get total stats
    const { data: allTranscripts } = await this.supabase.from('transcripts')
  ...
  async getScheduledReports(): Promise<ScheduledReport[]> {
    ...
    const { data, error } = await this.supabase.from('scheduled_reports')
      .select('*')
      .eq('user_id', user.id)
  ...
  async deleteScheduledReport(id: string): Promise<void> {
    const { error } = await this.supabase.from('scheduled_reports')
      .delete()
      .eq('id', id);
