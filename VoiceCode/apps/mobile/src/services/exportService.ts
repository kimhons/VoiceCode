...
export class ExportService {
  ...
  private async exportToPDF(transcript: Transcript, options: ExportOptions): Promise<void> {
    ...
    doc.text(transcript.title || 'Untitled', margin, yPosition);
    yPosition += 10;
    ...
    if (options.includeMetadata) {
      ...
      yPosition += 6;
      doc.text(`Language: ${(transcript.language || 'en').toUpperCase()}`, margin, yPosition);
      yPosition += 6;
      doc.text(`Mode: ${transcript.professional_mode || 'general'}`, margin, yPosition);
      yPosition += 6;
      doc.text(`Duration: ${this.formatDuration(transcript.duration)}`, margin, yPosition);
      yPosition += 6;
      doc.text(`Words: ${transcript.word_count || 0}`, margin, yPosition);
      yPosition += 6;
      if (options.includeConfidence) {
        doc.text(`Confidence: ${((transcript.confidence || 0) * 100).toFixed(1)}%`, margin, yPosition);
        yPosition += 6;
      }
      yPosition += 4;
    }
    ...
    // Save
    doc.save(`${this.sanitizeFilename(transcript.title || 'Untitled')}.pdf`);
  }
  ...
  private async exportToDOCX(transcript: Transcript, options: ExportOptions): Promise<void> {
      ...
      paragraphs.push(
        ...
        new Paragraph({
          children: [
            ...
            new TextRun((transcript.language || 'en').toUpperCase()),
          ],
          spacing: { after: 100 },
        }),
        new Paragraph({
          children: [
            ...
            new TextRun(transcript.professional_mode || 'general'),
          ],
          spacing: { after: 100 },
        }),
        new Paragraph({
        ...
        new Paragraph({
          children: [
            ...
            new TextRun((transcript.word_count || 0).toString()),
          ],
          spacing: { after: 200 },
        })
      );
        ...
        paragraphs.push(
          new Paragraph({
            children: [
              ...
              new TextRun(`${((transcript.confidence || 0) * 100).toFixed(1)}%`),
            ],
            spacing: { after: 200 },
          })
        );
    ...
    // Save
    const blob = await (doc as any).toBlob();
    saveAs(blob, `${this.sanitizeFilename(transcript.title || 'Untitled')}.docx`);
  }
  ...
  private async exportToTXT(transcript: Transcript, options: ExportOptions): Promise<void> {
    ...
    if (options.includeMetadata) {
      ...
      content += `Language: ${(transcript.language || 'en').toUpperCase()}\n`;
      ...
      if (options.includeConfidence && transcript.confidence !== undefined) {
    ...
    saveAs(blob, `${this.sanitizeFilename(transcript.title || 'Untitled')}.txt`);
  }
  ...
  private async exportToSRT(transcript: Transcript, options: ExportOptions): Promise<void> {
    ...
    // Save
    const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
    saveAs(blob, `${this.sanitizeFilename(transcript.title || 'Untitled')}.srt`);
  }
  ...
  private async exportToVTT(transcript: Transcript, options: ExportOptions): Promise<void> {
    ...
    saveAs(blob, `${this.sanitizeFilename(transcript.title || 'Untitled')}.vtt`);
  }
  ...
  private async exportToJSON(transcript: Transcript, options: ExportOptions): Promise<void> {
    ...
    saveAs(blob, `${this.sanitizeFilename(transcript.title || 'Untitled')}.json`);
  }
