...
class VideoTranscriptionService {
  ...
  async uploadVideo(
    ...
    try {
      ...
      // Note: Supabase storage doesn't support onUploadProgress in the upload options
      // Progress tracking would need to be implemented differently (e.g., chunked uploads)
      if (onProgress) {
        onProgress({
          loaded: file.size,
          total: file.size,
  ...
  private async transcribeAudio(videoId: string, _audioUrl: string): Promise<string> {
    try {
      const supabaseService = getSupabaseService();
      const client = supabaseService.getClient();
      if (!client) throw new Error('Supabase client not available');
