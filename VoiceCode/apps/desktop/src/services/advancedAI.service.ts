...
class AdvancedAIService {
  ...
  async executeCustomPrompt(promptId: string, transcriptText: string): Promise<string> {
    try {
      const supabaseService = getSupabaseService();
      const client = supabaseService.getClient();
      if (!client) throw new Error('Supabase client not available');
    ...
    try {
      const response = await fetch(`${this.apiUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          model: 'gpt-4o-mini',
          messages: [
            { role: 'system', content: 'You are a helpful AI assistant that provides accurate, concise responses.' },
