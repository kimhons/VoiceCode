...
// Internal transcript type for analytics queries
interface AnalyticsTranscript {
	id: string;
	duration?: number;
	word_count?: number;
	accuracy?: number;
...
export interface PerformanceAnalytics {
	...
	performanceByMode: Array<{
		mode: string;
		avgTime: number;
		successRate: number;
	}>;
	performanceByLanguage: Array<{
		language: string;
		avgTime: number;
		accuracy: number;
	}>;
...
class AdvancedAnalyticsService {
	...
	async getUsageAnalytics(
		...
		try {
			...
			const { data: rawTranscripts, error } = await client
				.from("transcripts")
				.select("*")
				.eq("user_id", userId)
			...
			const transcripts = (rawTranscripts || []) as AnalyticsTranscript[];

			// Calculate metrics
			const totalTranscripts = transcripts.length;
			const totalDuration =
				transcripts.reduce(
					(sum: number, t: AnalyticsTranscript) => sum + (t.duration || 0),
					0,
				) || 0;
			const totalWords =
				transcripts.reduce(
					(sum: number, t: AnalyticsTranscript) => sum + (t.word_count || 0),
					0,
				) || 0;
			const averageAccuracy =
				transcripts.reduce(
					(sum: number, t: AnalyticsTranscript) => sum + (t.accuracy || 0),
					0,
				) / totalTranscripts || 0;
			...
			transcripts.forEach((t: AnalyticsTranscript) => {
	...
	async getCostAnalytics(
		...
		try {
			...
			// Get cost data
			const { data: rawCosts, error } = await client
				.from("cost_tracking")
				.select("*")
				.eq("user_id", userId)
				.gte("created_at", startDate.toISOString())
			...
			const totalCost = costs.reduce(
				(sum: number, c) => sum + (c.cost || 0),
				0,
			);
			const totalTranscripts = costs.length || 1;
			const totalMinutes =
				costs.reduce((sum: number, c) => sum + (c.duration || 0), 0) / 60 || 1;
			const totalWords =
				costs.reduce((sum: number, c) => sum + (c.word_count || 0), 0) || 1;
			...
			costs.forEach((c) => {
				...
				categoryTotals.set(
					category,
					(categoryTotals.get(category) || 0) + (c.cost || 0),
				);
			});
			...
			const savingsOpportunities = [
				...
				},
				{
					...
					potentialSavings: projectedMonthlyCost * 0.1,
				},
				{
	...
	async getPerformanceAnalytics(
		...
		try {
			...
			const { data: rawTranscripts2, error } = await client
				.from("transcripts")
				.select("*")
				.eq("user_id", userId)
				.gte("created_at", startDate.toISOString())
			...
			if (error) throw error;
			const transcripts = (rawTranscripts2 || []) as AnalyticsTranscript[];

			const total = transcripts.length || 1;
			const successful =
				transcripts.filter((t: AnalyticsTranscript) => t.status === "completed")
					.length || 0;
			const failed =
				transcripts.filter((t: AnalyticsTranscript) => t.status === "failed")
					.length || 0;

			const averageTranscriptionTime =
				transcripts.reduce(
					(sum: number, t: AnalyticsTranscript) =>
						sum + (t.processing_time || 0),
					0,
				) / total || 0;
			const averageProcessingSpeed =
				transcripts.reduce((sum: number, t: AnalyticsTranscript) => {
			...
			const averageConfidence =
				transcripts.reduce(
					(sum: number, t: AnalyticsTranscript) => sum + (t.confidence || 0),
					0,
				) / total || 0;
			...
			const performanceByMode = Array.from(modeStats.entries()).map(
				([mode, stats]) => ({
					mode,
			...
			const langStats = new Map<
				string,
				{ times: number[]; accuracies: number[] }
			>();
			transcripts.forEach((t: AnalyticsTranscript) => {
				const lang = t.language || "en";
			...
			const performanceByLanguage = Array.from(langStats.entries()).map(
				([language, stats]) => ({
					language,
					avgTime: stats.times.reduce((a, b) => a + b, 0) / stats.times.length,
					accuracy:
	...
	private async getHistoricalData(
		...
		// In production, query actual analytics backend; mock only in dev (GAP-MOCK)
	...
	private detectAnomalies(
		dataPoints: Array<{ date: string; value: number }>,
	): Array<{ date: string; value: number; reason: string }> {
