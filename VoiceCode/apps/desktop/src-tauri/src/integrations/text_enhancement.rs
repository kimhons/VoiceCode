...
impl From<EnhancedContext> for EnhancementContext {
    fn from(ctx: EnhancedContext) -> Self {
        EnhancementContext {
            domain: ctx.domain.unwrap_or_else(|| "general".to_string()),
            audience: ctx.audience.unwrap_or_else(|| "general".to_string()),
            purpose: ctx.purpose.unwrap_or_else(|| "general".to_string()),
...
impl From<EnhancedProcessingOptions> for EnhancementOptions {
    fn from(opts: EnhancedProcessingOptions) -> Self {
        EnhancementOptions {
            ...
            maintain_length: !opts.generate_alternatives,
        }
    }
}
...
impl TextEnhancer {
    ...
    pub fn new(client: Arc<Mutex<AIMLClient>>, model: String) -> Self {
        Self {
            client,
            model,
            enhancement_cache: tokio::sync::Mutex::new(lru::LruCache::new(std::num::NonZeroUsize::new(100).unwrap())), // Cache 100 results
        }
    }
    ...
    pub async fn enhance_text(&self, request: EnhancementRequest) -> Result<EnhancementResult, AIMLError> {
        ...
        if let Some(choice) = response.choices.first() {
            ...
            // Generate cache key before moving request
            let cache_key = self.generate_cache_key(&request);
            ...
            let result = EnhancementResult {
                ...
                improvements,
                processing_time_ms: processing_time.try_into().unwrap_or(0),
    ...
    pub async fn summarize_text(&self, text: String) -> Result<SummarizationResult, AIMLError> {
        ...
        let messages = vec![
            AIMLMessage {
                role: "system".to_string(),
                content: "You are an expert summarizer. Create a concise, informative summary that captures the main points and key details. Format the summary clearly and include bullet points for key insights.".to_string(),
            },
            AIMLMessage {
                role: "user".to_string(),
        ...
        if let Some(choice) = response.choices.first() {
            ...
            Ok(SummarizationResult {
                ...
                summary,
                key_points,
                compression_ratio: 1.0 - (summary_len as f32 / text.len() as f32),
                confidence_score: 0.85,
                estimated_reading_time_seconds: (summary_len / 200) as u32, // Assuming 200 chars per minute
            })
        } else {
    ...
    pub async fn analyze_text(&self, text: String) -> Result<TextAnalysis, AIMLError> {
        ...
        let messages = vec![
            AIMLMessage {
                role: "system".to_string(),
                content: "You are a text analysis expert. Analyze the given text and provide detailed insights about readability, grammar, structure, sentiment, and suggestions for improvement. Return your analysis in a structured JSON format.".to_string(),
            },
            AIMLMessage {
                role: "user".to_string(),
        ...
        if let Some(choice) = response.choices.first() {
            ...
            match self.parse_text_analysis(analysis_text, &text) {
                ...
                Err(_) => {
                    ...
                    Ok(self.basic_text_analysis(text, processing_time.try_into().unwrap_or(0)))
                }
            }
        } else {
    ...
    fn build_enhancement_instructions(&self, request: &EnhancementRequest) -> String {
        ...
        if request.options.improve_clarity {
            instructions.push("• Improve clarity by rephrasing complex sentences and eliminating ambiguity".to_string());
        }
        if request.options.fix_grammar {
            instructions.push("• Fix grammar, punctuation, and spelling errors".to_string());
        }
        if request.options.enhance_style {
            instructions.push("• Enhance writing style to be more engaging and professional".to_string());
        }
        ...
        if request.options.remove_redundancy {
            instructions.push("• Remove redundant and repetitive content".to_string());
        }
        if request.options.improve_readability {
            instructions.push("• Improve readability through better sentence structure and word choice".to_string());
        }
        if request.options.preserve_meaning {
            instructions.push("• Preserve the original meaning and intent".to_string());
        }
