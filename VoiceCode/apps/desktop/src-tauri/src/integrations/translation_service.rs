...
impl Translator {
    ...
    pub fn new(client: Arc<Mutex<AIMLClient>>, model: String) -> Self {
        Self {
            client,
            model,
            translation_cache: tokio::sync::Mutex::new(lru::LruCache::new(std::num::NonZeroUsize::new(200).unwrap())), // Cache 200 translations
    ...
    pub async fn translate(&self, request: TranslationRequest) -> Result<TranslationResult, AIMLError> {
        ...
        if let Some(choice) = response.choices.first() {
            ...
            // Generate quality recommendations before moving quality
            let quality_recommendations = self.generate_quality_recommendations(&quality);
            let overall_score = quality.overall_score;
    ...
    pub async fn detect_language(&self, text: &str) -> Result<String, AIMLError> {
        ...
        let messages = vec![
            super::ai_ml_core::AIMLMessage {
                role: "system".to_string(),
                content: "You are a language detection expert. Identify the language of the given text and respond with only the ISO 639-1 language code (e.g., 'en', 'es', 'fr').".to_string(),
            },
            super::ai_ml_core::AIMLMessage {
                role: "user".to_string(),
    ...
    async fn apply_enhancement(&self, mut result: TranslationResult, request: &EnhancedTranslationRequest) -> Result<TranslationResult, AIMLError> {
        ...
        let messages = vec![
            super::ai_ml_core::AIMLMessage {
                role: "system".to_string(),
                content: "You are an expert editor specializing in translation enhancement.".to_string(),
            },
            super::ai_ml_core::AIMLMessage {
                role: "user".to_string(),
