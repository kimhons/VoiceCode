...
impl From<ApiEnhancedContext> for EnhancedContext {
    fn from(ctx: ApiEnhancedContext) -> Self {
...
impl ContextProcessor {
    ...
    pub fn new(client: Arc<Mutex<AIMLClient>>, model: String) -> Self {
        Self {
            client,
            model,
            context_cache: tokio::sync::Mutex::new(lru::LruCache::new(std::num::NonZeroUsize::new(150).unwrap())), // Cache 150 contexts
        ...
        if let Some(choice) = response.choices.first() {
            ...
            let result = ContextAwareResult {
                ...
                metadata: ContextMetadata {
                    ...
                    memory_utilized: request.context.conversation_history.len(),
    ...
    pub async fn analyze_conversation_flow(&self, messages: Vec<String>) -> Result<ConversationFlow, AIMLError> {
        ...
        let messages = vec![
            super::ai_ml_core::AIMLMessage {
                role: "system".to_string(),
                content: "You are an expert conversation analyst. Analyze the conversation flow, coherence, and engagement patterns. Provide detailed insights about the conversation quality and user interaction patterns.".to_string(),
            },
            super::ai_ml_core::AIMLMessage {
                role: "user".to_string(),
    ...
    pub async fn predict_intent(&self, text: String, context: &EnhancedContext) -> Result<UserIntent, AIMLError> {
        ...
        let messages = vec![
            super::ai_ml_core::AIMLMessage {
                role: "system".to_string(),
                content: "You are an expert intent classifier. Analyze user text and classify the primary intent.".to_string(),
            },
            super::ai_ml_core::AIMLMessage {
                role: "user".to_string(),
    ...
    async fn update_conversation_memory(&self, request: &ContextAwareRequest) {
        ...
        let messages_len = memory.messages.len();
        if messages_len > 100 {
            memory.messages.drain(0..messages_len - 100);
        }
    }
    ...
    async fn extract_topic(&self, text: &str) -> Result<String, AIMLError> {
        ...
        let messages = vec![
            super::ai_ml_core::AIMLMessage {
                role: "system".to_string(),
                content: "Extract the main topic of this text in one or two words. Respond with only the topic.".to_string(),
            },
            super::ai_ml_core::AIMLMessage {
                role: "user".to_string(),
