...
impl VoiceGenerator {
    ...
    pub fn new(client: Arc<Mutex<AIMLClient>>, model: String) -> Self {
        Self {
            ...
            synthesis_cache: tokio::sync::Mutex::new(lru::LruCache::new(std::num::NonZeroUsize::new(50).unwrap())), // Cache 50 results
        }
    }
    ...
    pub async fn generate_voice(&self, request: VoiceRequest) -> Result<VoiceResult, AIMLError> {
        ...
        let voice_config = self.prepare_voice_config(&request.voice_config, &request.audio_settings);
        
        // Generate SSML if enabled
        let processed_text = if request.voice_config.ssml_enabled {
            self.generate_ssml(&request.text, &request.voice_config.voice_characteristics)?
        } else {
            request.text.clone()
        };
        ...
        let audio_data = client.generate_voice(
            ...
            super::ai_ml_core::VoiceConfig {
                ...
                speed: Some(request.voice_config.voice_characteristics.speaking_rate),
                pitch: Some(request.voice_config.voice_characteristics.pitch),
            }
        ).await?;
        ...
        let duration_seconds = self.estimate_duration(&request.text, &request.voice_config.voice_characteristics);
        ...
        let result = VoiceResult {
            ...
            metadata: VoiceMetadata {
                ...
                api_response_time_ms: processing_time.try_into().unwrap_or(0),
            },
        };
            ...
            let variation_request = VoiceRequest {
                ...
                voice_config: VoiceConfig {
                    ...
                    voice_characteristics: VoiceCharacteristics {
                        speaking_rate: request.voice_config.voice_characteristics.speaking_rate * (0.9 + (i as f32 * 0.05)), // Slight variations
                        pitch: request.voice_config.voice_characteristics.pitch + (i as f32 - variations as f32 / 2.0) * 2.0,
                        volume: request.voice_config.voice_characteristics.volume,
                        emphasis: request.voice_config.voice_characteristics.emphasis,
                        style: request.voice_config.voice_characteristics.style.clone(),
                        emotion: request.voice_config.voice_characteristics.emotion.clone(),
                    },
    ...
    pub async fn batch_synthesize(&self, requests: Vec<VoiceRequest>) -> Result<Vec<VoiceResult>, AIMLError> {
        ...
        // Note: For true parallel processing, we'd need to restructure to avoid lifetime issues
        for request in requests {
            let result = self.generate_voice(request).await?;
            results.push(result);
        }
    ...
    fn prepare_voice_config(&self, config: &VoiceConfig, audio_settings: &AudioSettings) -> super::ai_ml_core::VoiceConfig {
        super::ai_ml_core::VoiceConfig {
            model: config.model.clone(),
            voice_id: config.voice_id.clone().unwrap_or_else(|| self.default_voice.clone()),
            output_format: self.get_format_string(&audio_settings.output_format),
            speed: Some(config.voice_characteristics.speaking_rate),
            pitch: Some(config.voice_characteristics.pitch),
        }
    }
    ...
    fn generate_cache_key(&self, request: &VoiceRequest) -> String {
        ...
        request.voice_config.voice_characteristics.speaking_rate.to_bits().hash(&mut hasher);
