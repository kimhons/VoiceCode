...
impl AIMLAPIGateway {
    ...
    pub async fn process_enhanced_text(&self, request: EnhancedTextRequest) -> AIMLResponse<EnhancedTextResult> {
        ...
        if let Some(context) = self.get_context_for_request(&request).await {
            if let Ok(context_result) = self.process_context(context).await {
                suggestions.extend(context_result.suggestions.into_iter().map(|s| s.description));
                confidence_scores.insert("context".to_string(), context_result.sentiment.confidence);
            }
        }
        ...
        // Calculate tokens before moving processed_text
        let tokens_consumed = self.estimate_tokens(&processed_text);
    ...
    pub async fn generate_enhanced_voice(&self, request: EnhancedVoiceRequest) -> Result<VoiceResult, AIMLError> {
        use super::voice_generation::{VoiceRequest, VoiceConfig, VoiceCharacteristics, VoiceStyle, VoiceEmotion, AudioSettings, AudioFormat, AudioQuality, VoiceProcessingOptions};

        // Convert EnhancedVoiceRequest to VoiceRequest
        let voice_request = VoiceRequest {
            id: request.id,
            text: request.text,
            voice_config: VoiceConfig {
                model: request.voice_config.model,
                ...
                voice_characteristics: VoiceCharacteristics {
                    speaking_rate: request.speed.unwrap_or(1.0),
                ...
                ssml_enabled: request.voice_config.apply_ssml,
            },
            audio_settings: AudioSettings {
                output_format: match request.output_format {
                    VoiceOutputFormat::MP3 { bitrate } => AudioFormat::MP3,
                    VoiceOutputFormat::WAV { sample_rate } => AudioFormat::WAV,
                    VoiceOutputFormat::OGG { quality } => AudioFormat::OGG,
                    VoiceOutputFormat::FLAC { compression_level } => AudioFormat::FLAC,
                },
                sample_rate: 22050,
                bitrate: 128,
                channels: 1,
                quality_level: match request.voice_config.quality_level {
                    VoiceQuality::Low => AudioQuality::Low,
                    VoiceQuality::Medium => AudioQuality::Medium,
                    VoiceQuality::High => AudioQuality::High,
                    VoiceQuality::Ultra => AudioQuality::Ultra,
                },
            },
            processing_options: VoiceProcessingOptions {
                apply_noise_reduction: request.post_processing.contains(&VoicePostProcessing::NoiseReduction),
                normalize_audio: request.post_processing.contains(&VoicePostProcessing::AudioNormalization),
                remove_silence: false,
                enhance_clarity: false,
                dynamic_range_compression: request.post_processing.contains(&VoicePostProcessing::DynamicRangeCompression),
        ...
        let generator = self.voice_generator.lock().await;
        generator.generate_voice(voice_request).await
    }
        ...
        match operation {
            ...
            TextOperation::Translate => {
                if let Some(target_lang) = &request.target_language {
                    use super::translation_service::{TranslationRequest, TranslationContext, TranslationDomain, FormalityLevel, TranslationOptions};
    ...
    pub async fn check_health(&self) -> HealthStatus {
        ...
        // Check text enhancement service
        {
            let health_start = std::time::Instant::now();
            let enhancer = self.text_enhancer.lock().await;
            let is_healthy = enhancer.health_check().await.is_ok();
            let response_time = health_start.elapsed().as_millis() as u64;
            status.response_times.insert("text_enhancement".to_string(), response_time);
            status.text_enhancement_healthy = is_healthy;
            if !is_healthy {
                *status.error_counts.entry("text_enhancement".to_string()).or_insert(0) += 1;
            }
        }
        ...
        // Check voice generation service
        {
            let health_start = std::time::Instant::now();
            let generator = self.voice_generator.lock().await;
            let is_healthy = generator.health_check().await.is_ok();
            let response_time = health_start.elapsed().as_millis() as u64;
            status.response_times.insert("voice_generation".to_string(), response_time);
            status.voice_generation_healthy = is_healthy;
            if !is_healthy {
                *status.error_counts.entry("voice_generation".to_string()).or_insert(0) += 1;
            }
        }
        ...
        // Check translation service
        {
            let health_start = std::time::Instant::now();
            let translator = self.translator.lock().await;
            let is_healthy = translator.health_check().await.is_ok();
            let response_time = health_start.elapsed().as_millis() as u64;
            status.response_times.insert("translation".to_string(), response_time);
            status.translation_healthy = is_healthy;
            if !is_healthy {
                *status.error_counts.entry("translation".to_string()).or_insert(0) += 1;
            }
        }

        // Check context processing service
        {
            let health_start = std::time::Instant::now();
            let processor = self.context_processor.lock().await;
            let is_healthy = processor.health_check().await.is_ok();
            let response_time = health_start.elapsed().as_millis() as u64;
            status.response_times.insert("context_processing".to_string(), response_time);
            status.context_processing_healthy = is_healthy;
            if !is_healthy {
                *status.error_counts.entry("context_processing".to_string()).or_insert(0) += 1;
            }
        }
