...
impl ErrorBoundary {
        ...
        E: From<AppError> + std::fmt::Display,
    {
        ...
        let state = self.circuit_breaker_state.lock().await.clone();
        match state {
    ...
    pub async fn get_circuit_breaker_state(&self) -> CircuitBreakerState {
        self.circuit_breaker_state.lock().await.clone()
    }
    ...
    pub async fn get_error_stats(&self) -> ErrorStats {
        ...
        let recovery_attempts = *self.recovery_attempts.lock().await;
        let circuit_breaker_state = self.circuit_breaker_state.lock().await.clone();
...
/// Helper macro for error boundary protected operations that converts to String
#[macro_export]
macro_rules! with_error_boundary_string {
    ($boundary:expr, $operation:expr) => {
        $boundary.execute(|| $operation).await.map_err(|e| e.to_string())
    };
}
