...
export const VoiceRecording: React.FC<VoiceRecordingProps> = React.memo(({
  ...
  const [aiWorkerReady, setAIWorkerReady] = useState(false);
  ...
  // AI Worker integration - only initialize if AI processing is enabled
  const {
    isReady: isAIReady,
    ...
    onInit: (status) => {
      // Only show initializing state if we're actively trying to record
      // Don't show it on component mount
      if (status.status === 'complete') {
        setAIWorkerReady(true);
      }
    },
  ...
  const startRecording = useCallback(async () => {
    if (disabled || isInitializing || state.isRecording) return;

    // Only show initializing when actually starting to record
    setIsInitializing(true);
    announceToScreenReader('Initializing microphone...', 'polite');
