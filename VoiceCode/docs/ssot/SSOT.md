...
> Mode: CANONICAL | Surfaces: 12 | Modules: 14
...
### SHIP-SCOPE vs DEFERRED

**SHIP-SCOPE** — Required for v1.0 launch:

- Desktop app (Tauri): voice-to-code, STT, code intelligence, CLI, vision — DONE
- Web app: 37 routes, medical mode, collaboration, export — DONE
- Mobile app: 150+ screens, recording, AI features — PARTIAL (19 TS errors, payment TODO)
- API server: alert proxy with tests — DONE
- Database: 10 tables, RLS, 5 edge functions — DONE
- CI/CD: working pipeline with all quality gates — TODO (GAP-CI)

**DEFERRED (PROPOSED-\*)**:

- PROPOSED-EMPTY-SVC: Implement ai-processor and voice-engine services
- PROPOSED-SHARED-PKG: Populate shared packages (types, utils)
- PROPOSED-INFRA: Docker/cloud infrastructure
- PROPOSED-EXT-MERGE: Consolidate two VS Code extensions
- PROPOSED-MARKETPLACE: VS Code/App Store/Play Store publication
...
| MOD-INFRA      | CI/CD & Infrastructure   | —     | `.github/workflows/`, `docker/`                 | Push/PR events                        | Build artifacts, test reports, deploy        |
| MOD-DOCS       | Documentation            | —     | `docs/`                                         | SSOT edits                            | Blueprint Binder artifacts                   |
...
| GAP-0006      | 40+ root legacy docs                                                      | ~~A) Archive to `docs/archive/`~~                                   | No      | **RESOLVED** — docs archived or consolidated into SSOT                                                   |
| GAP-0008      | No root README.md                                                         | ~~A) Create README~~                                                | No      | **RESOLVED** — 121-line README created (TASK-INFRA-0004)                                                 |
| GAP-CI        | CI pipeline failures on main branch                                       | A) Fix failing jobs; B) Add missing quality gates                   | Yes     | OPEN — E2E smoke tests + security gating required                                                        |
| GAP-EMPTY-SVC | `ai-processor` and `voice-engine` services are empty stubs                | A) Implement; B) Remove from tree                                   | No      | OPEN — DEFERRED                                                                                          |
| GAP-MOCK      | Desktop FE uses mock data in production components                        | A) Replace with real Tauri IPC calls; B) Gate behind `__DEV__` flag | No      | OPEN                                                                                                     |
| GAP-3PM       | 3 package managers detected (npm + yarn + pnpm lockfiles)                 | A) Standardize on one; B) Add `.npmrc` enforcement                  | No      | OPEN                                                                                                     |
| GAP-2EXT      | Two VS Code extensions exist (`voiceflow-vscode` + `voicecode-mcp-agent`) | A) Merge into one; B) Clarify distinct scopes                       | No      | OPEN — DEFERRED                                                                                          |
| GAP-AUTH-API  | API server has no authentication middleware                               | A) Add Supabase JWT verification; B) Proxy-only (no auth needed)    | No      | OPEN                                                                                                     |
...
| ~~Archive legacy docs~~                          | Repository hygiene              | —               | ~~P2~~   | **DONE** (Wave 2) |
| ~~Fix CI pipeline (GAP-CI)~~                     | ~~Unblocks merge gating~~       | REQ-CI-\*       | ~~P0~~   | **DONE**          |
| ~~Fix mobile TS errors + add tests~~             | ~~Stabilize mobile platform~~   | REQ-MOBILE-\*   | ~~P0~~   | **DONE**          |
| Mobile Stripe payment                            | Enables subscriptions on mobile | REQ-MOBILE-0005 | P1       | TODO              |
| Add API request validation + auth (GAP-AUTH-API) | Input safety + access control   | REQ-API-\*      | P1       | TODO              |
| Replace mock data in desktop FE (GAP-MOCK)       | Production correctness          | REQ-DESKTOP-\*  | P1       | TODO              |
| Standardize package manager (GAP-3PM)            | Build reproducibility           | REQ-CI-\*       | P2       | TODO              |
| Implement empty services (GAP-EMPTY-SVC)         | Feature completeness            | —               | P3       | DEFERRED          |
| Consolidate VS Code extensions (GAP-2EXT)        | Developer experience            | —               | P3       | DEFERRED          |
...
| ------------------- | ---------------------------------- | ----------------------------------------------------------- |
| UI/UX Blueprint     | `docs/ssot/UI-UX-BLUEPRINT.md`     | Design system, component inventory, accessibility standards |
| Ownership Matrix    | `docs/ssot/OWNERSHIP.md`           | Module ownership, CODEOWNERS mapping, review policy         |
| Traceability Matrix | `docs/ssot/traceability_matrix.md` | REQ → TASK → file-path evidence                             |
| Health Metrics      | `docs/ssot/health_metrics.md`      | Quality gate pass/fail dashboard                            |
| Version History     | `docs/ssot/version_history.md`     | SSOT changelog                                              |

---

_All requirements derived from repository evidence. No hallucinated paths or features. DICE v3.6 compliant._
