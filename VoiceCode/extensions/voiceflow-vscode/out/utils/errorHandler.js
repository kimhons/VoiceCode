...
exports.getMessageFunction = getMessageFunction;
...
function parseError(error) {
    ...
    if (error instanceof Error) {
        ...
        if (errorCode && exports.ERROR_MESSAGES[errorCode]) {
            return exports.ERROR_MESSAGES[errorCode];
        }
        // Try to match error message to known patterns
        const message = error.message.toLowerCase();
        if (message.includes('microphone') || message.includes('permission')) {
            return exports.ERROR_MESSAGES.MICROPHONE_ACCESS_DENIED;
        }
        if (message.includes('network') || message.includes('offline')) {
            return exports.ERROR_MESSAGES.NETWORK_OFFLINE;
        }
        if (message.includes('timeout')) {
            return exports.ERROR_MESSAGES.NETWORK_TIMEOUT;
        }
        if (message.includes('auth') || message.includes('token')) {
            return exports.ERROR_MESSAGES.AUTH_FAILED;
        }
        if (message.includes('rate limit')) {
            return exports.ERROR_MESSAGES.AI_RATE_LIMITED;
        }
        if (message.includes('quota')) {
            return exports.ERROR_MESSAGES.AI_QUOTA_EXCEEDED;
        }
        // Return generic error with original message
        return {
            ...exports.ERROR_MESSAGES.UNKNOWN_ERROR,
            message: error.message,
        };
    }
    // Fallback for non-Error objects
    return exports.ERROR_MESSAGES.UNKNOWN_ERROR;
}
/**
 * Check if object is an ExtensionError
 */
function isExtensionError(obj) {
 ...
 */
function getMessageFunction(severity) {
    switch (severity) {
        case 'info':
 ...
 */
function createError(code, message, userMessage, options = {}) {
    return {
        code,
        message,
        userMessage,
