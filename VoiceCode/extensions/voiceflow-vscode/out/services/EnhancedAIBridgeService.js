...
class EnhancedAIBridgeService {
    ...
    _onRequestCompleted = new vscode.EventEmitter();
    _onStreamChunk = new vscode.EventEmitter();
    // Public event accessors
    onProviderChanged = this._onProviderChanged.event;
    ...
    detectAvailableProviders() {
        const providers = ['copilot', 'cursor', 'cline', 'aider', 'augment', 'anthropic', 'openai', 'local'];
        for (const provider of providers) {
            ...
            switch (provider) {
                ...
                case 'anthropic':
                    ...
                    status.configured = !!this.config.get('anthropicApiKey');
                    break;
                case 'openai':
                    status.available = true;
                    status.configured = !!this.config.get('openaiApiKey');
                    break;
                case 'local':
                    status.available = !!this.config.get('localLLMEndpoint');
                    status.configured = status.available;
                    break;
            }
     ...
     */
    async sendToAnthropic(request) {
        const apiKey = this.config.get('anthropicApiKey');
        if (!apiKey) {
            return { success: false, error: 'Anthropic API key not configured', provider: 'anthropic' };
        }
        const model = request.options?.model || this.config.get('anthropicModel', 'claude-3-5-sonnet-20241022');
        const maxTokens = request.options?.maxTokens || this.config.get('maxTokens', 4096);
        try {
     ...
     */
    async sendToLocal(request) {
        const endpoint = this.config.get('localLLMEndpoint');
        if (!endpoint) {
     ...
     */
    async sendViaLanguageModelAPI(request, preferredModelFamily) {
        try {
            ...
            if (request.context?.code) {
                messages.unshift(vscode.LanguageModelChatMessage.User(`Context - Working with ${request.context.language || 'code'} in file: ${request.context.filePath || 'unknown'}`));
            }
     ...
     */
    async sendToCopilotViaCommand(request) {
        try {
     ...
     */
    formatPromptForProvider(request, provider) {
     ...
     */
    async parseAnthropicResponse(data, request) {
...
exports.EnhancedAIBridgeService = EnhancedAIBridgeService;
exports.default = EnhancedAIBridgeService;
//# sourceMappingURL=EnhancedAIBridgeService.js.map