...
exports.CodebaseIndexService = void 0;
...
class CodebaseIndexService {
    ...
    disposables = [];
    // Event emitters
    _onIndexingStarted = new vscode.EventEmitter();
    _onIndexingCompleted = new vscode.EventEmitter();
    _onIndexingProgress = new vscode.EventEmitter();
    _onIndexingError = new vscode.EventEmitter();
    onIndexingStarted = this._onIndexingStarted.event;
    onIndexingCompleted = this._onIndexingCompleted.event;
    onIndexingProgress = this._onIndexingProgress.event;
    onIndexingError = this._onIndexingError.event;
    ...
    initializeSettings() {
        ...
        const apiKey = this.config.get('openaiApiKey') || process.env.OPENAI_API_KEY;
        if (apiKey) {
            llamaindex_1.Settings.embedModel = new llamaindex_1.OpenAIEmbedding({
                apiKey,
                model: this.config.get('embeddingModel', 'text-embedding-3-small'),
            });
        }
        // Configure chunk size for better context
        llamaindex_1.Settings.chunkSize = this.config.get('chunkSize', 512);
        llamaindex_1.Settings.chunkOverlap = this.config.get('chunkOverlap', 50);
    }
    /**
    ...
    setupFileWatcher() {
        ...
        this.fileWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(workspaceFolder, '**/*.{ts,js,tsx,jsx,py,java,cpp,c,go,rs,rb,php}'));
    ...
    async indexWorkspace() {
        ...
        try {
            ...
            // Find all code files
            const files = await vscode.workspace.findFiles('**/*.{ts,js,tsx,jsx,py,java,cpp,c,go,rs,rb,php}', '**/node_modules/**');
            ...
            const llamaDocuments = Array.from(this.documents.values()).map(doc => new llamaindex_1.Document({
                text: doc.content,
                metadata: {
            ...
            this.index = await llamaindex_1.VectorStoreIndex.fromDocuments(llamaDocuments);
            // Save index to disk
            ...
            this._onIndexingError.fire(error);
            this.telemetry.recordError(error, { context: 'codebase_indexing' });
            throw error;
        }
        finally {
    ...
    async semanticSearch(query, topK = 5) {
        ...
        try {
            ...
            this.telemetry.recordError(error, { context: 'semantic_search', query });
            throw error;
        }
    }
    /**
     ...
     */
    extractSnippet(text, query, contextLines = 5) {
     ...
     */
    calculateRelevanceScore(line, query) {
        let score = 0;
    ...
    async loadIndex() {
        try {
            const indexData = this.context.globalState.get('codebaseIndex');
            if (!indexData) {
...
exports.CodebaseIndexService = CodebaseIndexService;
exports.default = CodebaseIndexService;
//# sourceMappingURL=CodebaseIndexService.js.map