...
exports.AgentCommunicationHub = void 0;
...
class AgentCommunicationHub {
    ...
    // Event emitters
    _onMessageSent = new vscode.EventEmitter();
    _onResponseReceived = new vscode.EventEmitter();
    _onMultiAgentComplete = new vscode.EventEmitter();
    onMessageSent = this._onMessageSent.event;
    onResponseReceived = this._onResponseReceived.event;
    onMultiAgentComplete = this._onMultiAgentComplete.event;
    constructor(registry, aiBridge, agentFactory, config, telemetry) {
     ...
     */
    async routeToMultiple(task, count = 3, context) {
    ...
    async sendToAgent(agentId, message) {
        ...
        try {
            ...
            this.registry.recordUsage(agentId, response.success, duration, response.metadata.cost || 0, message.type);
            ...
            return this.createErrorResponse(message.id, agentId, error instanceof Error ? error.message : 'Unknown error', startTime);
        }
    }
    /**
     * Send to internal agent (our specialized agents)
     */
    async sendToInternalAgent(agent, message) {
        const startTime = Date.now();
     ...
     */
    async sendToExternalAgent(agent, message) {
        const startTime = Date.now();
     ...
     */
    async executeParallel(task, agentIds, context) {
        const startTime = Date.now();
     ...
     */
    async executeWithConsensus(task, agentIds, context) {
     ...
     */
    async executeRace(task, agentIds, context) {
        const promises = agentIds.map(agentId => {
            const message = {
                id: `msg-${Date.now()}-${agentId}`,
                from: 'voicecode-hub',
     ...
     */
    createErrorResponse(requestId, agentId, error, startTime) {
        return {
            id: `response-${Date.now()}`,
            requestId,
            from: agentId,
...
exports.AgentCommunicationHub = AgentCommunicationHub;
exports.default = AgentCommunicationHub;
