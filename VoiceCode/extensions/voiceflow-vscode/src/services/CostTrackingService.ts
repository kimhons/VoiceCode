...
export class CostTrackingService implements vscode.Disposable {
  ...
  // Event emitters
  private readonly _onUsageRecorded = new vscode.EventEmitter<TokenUsage>();
  private readonly _onBudgetAlert = new vscode.EventEmitter<BudgetAlert>();
  private readonly _onBudgetExceeded = new vscode.EventEmitter<BudgetAlert>();

  public readonly onUsageRecorded = this._onUsageRecorded.event;
  public readonly onBudgetAlert = this._onBudgetAlert.event;
  public readonly onBudgetExceeded = this._onBudgetExceeded.event;
  ...
  constructor(
    ...
    // Load configuration
    this.dailyBudget = this.config.get<number>('dailyBudgetLimit', 5.0); // $5/day default
    this.monthlyBudget = this.config.get<number>('monthlyBudgetLimit', 100.0); // $100/month default
   ...
   */
  private showBudgetNotification(alert: BudgetAlert): void {
  ...
  private showUsageDashboard(): void {
    ...
    const panel = vscode.window.createWebviewPanel(
      'voicecodeCostDashboard',
      'VoiceCode Cost Dashboard',
   ...
   */
  private generateDashboardHTML(summary: CostSummary): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
   ...
   */
  public getSummary(period: 'today' | 'week' | 'month' | 'all'): CostSummary {
    const now = new Date();
    let startDate: Date;
   ...
   * Calculate current costs
   */
  private calculateCurrentCosts(): void {
  ...
  private async loadUsageHistory(): Promise<void> {
    try {
      const saved = this.context.globalState.get<TokenUsage[]>('tokenUsageHistory');
      if (saved) {
