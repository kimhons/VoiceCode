 ...
 */
export class VoiceSettingsService implements vscode.Disposable {
  private config: vscode.WorkspaceConfiguration;
  private telemetry: TelemetryService;
  private disposables: vscode.Disposable[] = [];

  // Common setting mappings for voice commands
  private settingMappings: Map<string, string> = new Map([
    // Theme settings
  ...
  private valueMappings: Map<string, any> = new Map<string, any>([
    // Boolean values
    ['on', true as any],
    ['off', false as any],
    ['enabled', true as any],
  ...
  // Event emitters
  private readonly _onSettingChanged = new vscode.EventEmitter<SettingChangeResult>();
  private readonly _onVoiceCommandProcessed = new vscode.EventEmitter<VoiceCommandInterpretation>();

  public readonly onSettingChanged = this._onSettingChanged.event;
  public readonly onVoiceCommandProcessed = this._onVoiceCommandProcessed.event;
   ...
   */
  public async handleVoiceCommand(command: string): Promise<SettingChangeResult | string> {
      ...
      case 'change_setting':
        if (interpretation.settingKey && interpretation.settingValue !== undefined) {
          return await this.updateSetting(
            interpretation.settingKey,
            interpretation.settingValue,
            interpretation.scope || vscode.ConfigurationTarget.Global
          );
        }
        break;
   ...
   */
  private interpretVoiceCommand(command: string): VoiceCommandInterpretation {
   ...
   */
  private resolveSettingKey(settingName: string): string | undefined {
   ...
   */
  private resolveSettingValue(value: string, settingKey?: string): any {
   ...
   */
  private determineScope(command: string): vscode.ConfigurationTarget {
   ...
   */
  private validateSettingKey(key: string): { valid: boolean; error?: string } {
   ...
   */
  private validateSettingValue(key: string, value: any): { valid: boolean; error?: string } {
  ...
  public async updateSetting(
    ...
    try {
      ...
      const result: SettingChangeResult = {
        success: true,
        key,
        oldValue,
        newValue: value,
      ...
      vscode.window.showInformationMessage(
        `✓ Updated ${key} to ${JSON.stringify(value)}`
      );
      ...
      vscode.window.showErrorMessage(
        `Failed to update ${key}: ${result.error}`
      );
  ...
  public async updateSettings(
    ...
    const results: SettingChangeResult[] = [];
   ...
   */
  public async resetSetting(key: string): Promise<SettingChangeResult> {
   ...
   */
  private listCommonSettings(): string {
    const categories = [
      '**Theme Settings:**',
      '- theme, icon theme',
      '',
   ...
   */
  public searchSettings(keyword: string): Array<{ name: string; key: string }> {
